...
class Cell:
  ...
  def draw(self, SCREEN, has_pion, pion_color):
    ...
    if has_pion:
      pygame.draw.circle(
        SCREEN, pion_color,
        (self.body.centerx, self.body.centery), self.body.width /2 -self.padding
      )
  ...
  def paint(self, SCREEN):
    for x in range(self.X):
      for y in range(self.Y):
        ...
        pion_color = TEAMS[LIST_PIONS[self.cells[x][y]].team]["color"] if has_pion else None
