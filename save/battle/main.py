...
class Cell:
  def __init__(self, x, y, margin, gutter, size, hover, active):
    self.x = x
    self.y = y
    self.active = active
    self.padding = 5
  ...
  def def_color(self):
    if self.hover is not False and (self.x, self.y) == self.hover:
      return DARK
    elif self.x == self.active[0] and self.y == self.active[1]:
      return BLUE
    return RED
  ...
  def paint(self, SCREEN):
    for x in range(self.X):
      for y in range(self.Y):
        ...
        cell = Cell(x, y, self.margin, self.gutter, self.size, self.hover, self.active)
...
while playing:
  ...
  if board.collidepoint(mouse_pos):
    ...
    if clic and not clicking:
      clicking = True
      game.activate(hovered_cell_x, hovered_cell_y)
  else:
    game.hover = False
    if clic and not clicking:
      clicking = True
      game.activate(None,None)
